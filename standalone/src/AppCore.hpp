...
namespace AppContext {
  constexpr char standaloneName[] = "EmojiesStandalone";
...
std::unique_ptr<dotname::EmojiesLib> uniqueLib;
...
int handlesArguments (int argc, const char* argv[]) {
  try {
    ...
    if (!result.count ("omit")) {
      // uniqueLib = std::make_unique<dotname::EmojiesLib> ();
      uniqueLib = std::make_unique<dotname::EmojiesLib> (AppContext::assetsPath);
      for (int i = 0; i < 9; i++)
        LOG_I_STREAM << uniqueLib->getRandomEmoji ();
